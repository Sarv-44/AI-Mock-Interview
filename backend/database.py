...
def sanitize_study_step_snapshot(step_type, snapshot):
    if not isinstance(snapshot, dict):
        return {}

    if step_type != "mcq_quiz":
        return snapshot

    sanitized = {key: value for key, value in snapshot.items() if key != "quiz_items"}
    quiz_items = []
    for index, item in enumerate(snapshot.get("quiz_items") or []):
        choices = [
            " ".join(str(choice).split())
            for choice in (item.get("choices") or [])
            if " ".join(str(choice).split())
        ]
        if not choices:
            continue
        quiz_items.append(
            {
                "quiz_item_id": str(item.get("quiz_item_id") or f"quiz_item_{index + 1}"),
    ...
    sanitized["quiz_items"] = quiz_items
    return sanitized
...
def normalize_study_plan_step_row(row):
    ...
    snapshot = sanitize_study_step_snapshot(step_type, parse_json_field(row.get("snapshot_json"), {}))
    return {
        "step_id": row["step_id"],
        "plan_id": row["plan_id"],
...
def init_database():
    ...
    try:
        ...
        cursor.execute(
            """
            CREATE TABLE IF NOT EXISTS study_plan_quiz_results (
                id INT AUTO_INCREMENT PRIMARY KEY,
                ...
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                INDEX idx_study_plan_quiz_results_plan_id (plan_id),
                INDEX idx_study_plan_quiz_results_step_id (step_id),
                INDEX idx_study_plan_quiz_results_user_id (user_id),
                CONSTRAINT fk_study_plan_quiz_results_plan_id
                    FOREIGN KEY (plan_id) REFERENCES study_plans(plan_id)
                    ON DELETE CASCADE
            )
            """
...
def get_score_leaderboard(board="overall", limit=10, topic_id=None, current_user_id=None):
    ...
    minimum_interviews = 3
    timeframe_days = None
    filters = ["interviews.user_id IS NOT NULL"]
    params = []
    resolved_topic_id = str(topic_id or "").strip()

    topic_payload = None

    if resolved_board == "weekly":
        minimum_interviews = 2
        timeframe_days = 7
        filters.append("interviews.created_at >= (NOW() - INTERVAL 7 DAY)")
    elif resolved_board == "topic":
        if not resolved_topic_id:
            return {
                "success": False,
                "error": "Topic ID is required",
                "detail": "Choose a topic before requesting the topic leaderboard.",
            }
        topic_payload = get_topic_by_id(resolved_topic_id)
        if not topic_payload:
            return {
                "success": False,
                "error": "Selected topic does not exist",
                "detail": "The requested topic leaderboard could not be found.",
            }
        minimum_interviews = 2
        filters.append("COALESCE(interviews.session_mode, 'topic') = 'topic'")
        filters.append("interviews.topic = %s")
    ...
    ranking_query = f"""
        SELECT
            interviews.user_id,
            users.username,
            ROUND(AVG(interviews.final_score), 1) AS average_score,
            COUNT(*) AS interview_count,
            MAX(interviews.created_at) AS latest_session_at
        FROM interviews
        INNER JOIN users ON users.user_id = interviews.user_id
        WHERE {where_clause}
        GROUP BY interviews.user_id, users.username
        HAVING COUNT(*) >= %s
        ORDER BY average_score DESC, interview_count DESC, latest_session_at DESC, interviews.user_id ASC
    """
    ranking_params = tuple(params + [minimum_interviews])

    current_user_progress_query = None
    current_user_progress_params = None
    if current_user_id:
        current_user_filters = list(filters)
        current_user_filters.append("interviews.user_id = %s")
        current_user_progress_query = f"""
            SELECT COUNT(*) AS interview_count
            FROM interviews
            WHERE {" AND ".join(current_user_filters)}
        """
        current_user_progress_params = tuple(params + [current_user_id])
    ...
    if connection is None:
        return {
            "success": False,
            "error": "Database connection failed",
            "detail": "Leaderboard data is unavailable right now.",
        }

    cursor = None
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.execute(ranking_query, ranking_params)
        ranked_rows = cursor.fetchall() or []

        def anonymize_user(user_id_value):
            suffix = hashlib.sha256(str(user_id_value).encode("utf-8")).hexdigest()[:4].upper()
        ...
        for index, row in enumerate(ranked_rows, start=1):
            ...
            entry = {
                "rank": index,
                "user_id": row["user_id"],
                "display_name": row["username"] if is_current_user else anonymize_user(row["user_id"]),
        ...
        if current_user_progress_query and current_user_progress_params:
            cursor.execute(current_user_progress_query, current_user_progress_params)
            progress_row = cursor.fetchone() or {}
            current_user_progress = {
                "user_id": current_user_id,
                "interview_count": int(progress_row.get("interview_count") or 0),
        ...
        title = {
            ...
            "topic": f"{topic_payload['title']} leaderboard" if topic_payload else "Topic leaderboard",
        }[resolved_board]
        ...
        logger.exception("Database error loading leaderboard | board=%s topic_id=%s", resolved_board, topic_id)
        return {
            "success": False,
            "error": "Failed to load leaderboard",
            "detail": describe_database_error(exc) or "Leaderboard data could not be loaded right now.",
        }
    finally:
        if cursor:
            cursor.close()
...
def build_study_plan_progress(plan, steps):
    ...
    scored_steps = [
        step
        for step in steps
        if step["status"] == "done" and step.get("snapshot", {}).get("last_score") not in (None, "")
    ]
        ...
        phase_breakdown.append(
            {
                ...
                "average_score": average_numbers(
                    [
                        step.get("snapshot", {}).get("last_score")
        ...
        topic_progress.append(
            {
                ...
                "average_score": average_numbers(
                    [
                        item.get("snapshot", {}).get("last_score")
                        for item in topic_steps
                        if item["status"] == "done" and item.get("snapshot", {}).get("last_score") not in (None, "")
                    ]
                ),
            }
        )
...
def build_checkpoint_days(round_one_days, round_two_days):
    checkpoint_days = []
    previous_day = 0

    for round_one_day, round_two_day in zip(round_one_days, round_two_days):
        round_one_day = int(round_one_day or 1)
        round_two_day = max(round_one_day + 1, int(round_two_day or round_one_day + 1))
        midpoint_gap = max(1, round(round((round_two_day - round_one_day) * 0.45)))
        checkpoint_day = round_one_day + midpoint_gap
        checkpoint_day = max(round_one_day + 1, min(checkpoint_day, round_two_day - 1))

        if checkpoint_day <= previous_day and checkpoint_day < round_two_day - 1:
            checkpoint_day = min(round_two_day - 1, previous_day + 1)

        checkpoint_days.append(checkpoint_day)
        previous_day = checkpoint_day

    return checkpoint_days


def normalize_quiz_choice_text(value, fallback=""):
...
def clear_study_step_completion_snapshot(snapshot):
    if not isinstance(snapshot, dict):
        return {}
    ...
    return {key: value for key, value in snapshot.items() if key not in transient_keys}


def build_topic_checkpoint_quiz(topic_id, topic_title, topic_questions):
    source_questions = list(topic_questions or [])
    if not source_questions:
        source_questions = build_topic_round_questions(topic_id, 1)

    selected_questions = source_questions[: min(3, len(source_questions))]
    if not selected_questions:
        return []

    answer_pool = []
    for question in source_questions:
        for field_name in ["sample_answer", "ideal_answer"]:
            candidate = normalize_quiz_choice_text(question.get(field_name))
            if candidate:
                answer_pool.append((question.get("question_id"), candidate))

    generic_distractors = [
        f"Keep the answer very generic and avoid giving a concrete {topic_title} example.",
        f"List related {topic_title} buzzwords without explaining how they solve the prompt.",
        "Focus on definitions only and skip trade-offs, constraints, and when you would use it.",
        "Talk around the topic without answering the interviewer directly.",
    ]

    quiz_items = []
    for index, question in enumerate(selected_questions, start=1):
        question_id = str(question.get("question_id") or f"{topic_id}_checkpoint_{index}")
        question_text = str(question.get("question_text") or f"Checkpoint question {index}").strip()
        correct_choice = normalize_quiz_choice_text(
            question.get("ideal_answer") or question.get("sample_answer"),
            f"Define {topic_title} clearly, then support it with one concrete example and a practical trade-off.",
        )
        distractors = []
        seen_choices = {correct_choice}

        for candidate_question_id, candidate_text in answer_pool:
            if candidate_question_id == question_id or candidate_text in seen_choices:
                continue
            seen_choices.add(candidate_text)
            distractors.append(candidate_text)

        for fallback in generic_distractors:
            fallback_text = normalize_quiz_choice_text(fallback)
            if fallback_text and fallback_text not in seen_choices:
                seen_choices.add(fallback_text)
                distractors.append(fallback_text)

        randomizer = random.Random(f"{topic_id}:{question_id}:mcq_quiz")
        randomizer.shuffle(distractors)
        options = distractors[:3] + [correct_choice]
        randomizer.shuffle(options)

        quiz_items.append(
            {
                "quiz_item_id": f"{question_id}_quiz",
                "question_id": question_id,
                "topic_id": topic_id,
                "prompt": f"Which answer best fits this checkpoint prompt: {question_text}",
                "choices": options,
                "correct_choice_index": options.index(correct_choice),
                "explanation": normalize_quiz_choice_text(
                    question.get("ideal_answer") or question.get("sample_answer"),
                    "The strongest answer should be direct, concrete, and tied back to interview reasoning.",
                ),
            }
        )
...
def build_study_plan_outline(role, target_days):
    ...
    phase_two_end = max(phase_one_end + 4, min(target_days - 1, round(target_days * 0.8)))
    ...
    checkpoint_enabled = bool(selected_topics)
    latest_round_two_end = max(phase_one_end + 2, phase_two_end - max(2, mixed_count))
    phase_two_round_end = max(
        phase_one_end + 2,
        min(
            latest_round_two_end,
            phase_one_end + round(max(3, phase_two_end - phase_one_end) * 0.62),
        ),
    )
    ...
    round_two_start = phase_one_end + 1
    round_two_days = build_spaced_days(round_two_start, max(round_two_start, phase_two_round_end), len(selected_topics))
    checkpoint_days = build_checkpoint_days(round_one_days, round_two_days) if checkpoint_enabled else []
    ...
    for index, topic in enumerate(selected_topics):
        ...
        checkpoint_quiz_items = build_topic_checkpoint_quiz(topic_id, topic_title, topic_questions or round_one_questions)
        ...
        if checkpoint_enabled and checkpoint_quiz_items:
            checkpoint_question_ids = [
                str(item.get("question_id")).strip()
                for item in checkpoint_quiz_items
                if str(item.get("question_id") or "").strip()
            ]
            checkpoint_day = checkpoint_days[index] if index < len(checkpoint_days) else max(phase_one_end + 1, round_two_days[index] - 1)
            steps.append(
                {
                    "step_id": build_study_step_id(),
                    "sequence_no": sequence_no,
                    ...
                    "step_type": "mcq_quiz",
                    "scheduled_day": checkpoint_day,
                    "scheduled_label": f"Day {checkpoint_day}",
                    "topic_id": topic_id,
                    "topic_title": topic_title,
                    "role_id": role["role_id"],
                    "role_title": role["title"],
                    ...
                    "status": "planned",
                    "snapshot": {
                        "headline": f"{topic_title} checkpoint quiz",
                        "description": f"Run a quick multiple-choice checkpoint before the harder {topic_title} round lands.",
                        "question_count": len(checkpoint_quiz_items),
                        "stage_label": "Checkpoint quiz",
                        "difficulty_focus": "Fast recall and pattern recognition",
                        "practice_prompts": [item["prompt"] for item in checkpoint_quiz_items[:2]],
                        "recommended_duration": 8,
                        "launch_mode": "mcq_quiz",
                        "can_launch_quiz": True,
                        "score_target": 80,
                        "quiz_items": checkpoint_quiz_items,
                    },
                }
            )
            sequence_no += 1
    ...
    summary = {
        ...
        "mcq_quiz_count": len(checkpoint_days) if checkpoint_enabled else 0,
...
def update_study_plan_step_status(user_id, plan_id, step_id, status):
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT
                study_plan_steps.snapshot_json,
                study_plan_steps.step_type
            FROM study_plan_steps
            ...
            """,
            (resolved_step_id, resolved_plan_id, resolved_user_id),
        )
        row = cursor.fetchone()
        if not row:
            connection.rollback()
        ...
        if normalized_status == "planned":
            cleared_snapshot = clear_study_step_completion_snapshot(parse_json_field(row.get("snapshot_json"), {}))
            cursor.execute(
                """
                ...
                (
                    normalized_status,
                    json.dumps(cleared_snapshot, default=str),
                    resolved_step_id,
                    resolved_plan_id,
                    resolved_user_id,
                ),
...
def save_study_plan_quiz_result(user_id, plan_id, step_id, answers):
    resolved_user_id = str(user_id or "").strip()
    resolved_plan_id = str(plan_id or "").strip()
    resolved_step_id = str(step_id or "").strip()
    if not resolved_user_id or not resolved_plan_id or not resolved_step_id:
    ...
    try:
        ...
        if str(row.get("step_type") or "").strip() != "mcq_quiz":
            return {"success": False, "error": "This step is not an MCQ checkpoint."}

        snapshot = parse_json_field(row.get("snapshot_json"), {})
        quiz_items = list(snapshot.get("quiz_items") or [])
        if not quiz_items:
            return {"success": False, "error": "This checkpoint quiz is unavailable right now."}

        if not isinstance(answers, list) or len(answers) != len(quiz_items):
            return {"success": False, "error": "Answer every checkpoint question before submitting."}

        normalized_answers = []
        for answer in answers:
            try:
                normalized_answers.append(int(answer))
            except (TypeError, ValueError):
                return {"success": False, "error": "Checkpoint answers must be valid option indexes."}

        review_items = []
        correct_answers = 0
        for index, quiz_item in enumerate(quiz_items):
            choices = [
                normalize_quiz_choice_text(choice)
                for choice in (quiz_item.get("choices") or [])
                if normalize_quiz_choice_text(choice)
            ]
            if not choices:
                return {"success": False, "error": "This checkpoint quiz is unavailable right now."}

            correct_index = int(quiz_item.get("correct_choice_index") or 0)
            selected_index = normalized_answers[index]
            is_correct = 0 <= selected_index < len(choices) and selected_index == correct_index
            if is_correct:
                correct_answers += 1

            review_items.append(
                {
                    "quiz_item_id": str(quiz_item.get("quiz_item_id") or f"quiz_item_{index + 1}"),
                    "prompt": str(quiz_item.get("prompt") or "").strip(),
                    "selected_index": selected_index,
                    "selected_choice": choices[selected_index] if 0 <= selected_index < len(choices) else None,
                    "correct_index": correct_index,
                    "correct_choice": choices[correct_index] if 0 <= correct_index < len(choices) else None,
                    "is_correct": is_correct,
                    "explanation": normalize_quiz_choice_text(
                        quiz_item.get("explanation"),
                        "The strongest answer should be direct, accurate, and grounded in interview reasoning.",
                    ),
                }
            )
        ...
        score_total = float(snapshot.get("score_total") or 0) + score
        completed_at = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        snapshot.update(
            {
                "last_score": score,
                "best_score": max(int(snapshot.get("best_score") or 0), score),
                ...
                "last_quiz_review": review_items,
            }
        )

        cursor.execute(
            """
            INSERT INTO study_plan_quiz_results (
                result_id,
                plan_id,
                step_id,
                user_id,
        ...
        cursor.execute(
            ...
            WHERE study_plan_steps.step_id = %s
              ...
              AND study_plans.user_id = %s
            """,
            (json.dumps(snapshot, default=str), resolved_step_id, resolved_plan_id, resolved_user_id),
        )
        cursor.execute(
            """
            SELECT
        ...
        connection.rollback()
        return database_failure(
            "Failed to save study quiz result",
            exc,
            detail="The checkpoint quiz result could not be stored right now.",
            plan_id=resolved_plan_id,
            step_id=resolved_step_id,
        )
    finally:
